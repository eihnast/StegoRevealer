...
internal class Program
{
    ...
    {
        if (!IsParametersValid(args, InputParameters.ToArray()))
            return;
        var alphaParameterName = GetSpecifiedParameter(args, InputParameters[0].AvailableNames);
        Alpha = TryGetDoubleParameter(args, alphaParameterName ?? "") ?? 0.05;
    ...
    private static double? TryGetDoubleParameter(string[] args, string parameterName)
    {
    ...
    private static string? GetSpecifiedParameter(string[] args, IEnumerable<string> avaliableParameterNames) => 
        args.FirstOrDefault(arg => avaliableParameterNames.Contains(arg));

    private static bool IsParametersValid(string[] args, params InputParameter[] inputParameters)
    {
