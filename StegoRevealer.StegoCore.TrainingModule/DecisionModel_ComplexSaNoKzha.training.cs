...
namespace StegoRevealer_StegoCore_TrainingModule
{
    ...
    {
        public const string RetrainFilePath =  @"E:\Temp\Steganalysis\Данные для машинного обучения\Файлы данных для обучения\Для загрузки в StegoRevealer\MlAnalysisData_ComplexSaNoKzha.csv";
        ...
        public static IEstimator<ITransformer> BuildPipeline(MLContext mlContext)
        {
            ...
            var pipeline = mlContext.Transforms.ReplaceMissingValues(new []{new InputOutputColumnPair(@"ChiSqrHorizontalRelativeVolume", @"ChiSqrHorizontalRelativeVolume"),new InputOutputColumnPair(@"ChiSqrVerticalRelativeVolume", @"ChiSqrVerticalRelativeVolume"),new InputOutputColumnPair(@"RsRelativeVolume", @"RsRelativeVolume"),new InputOutputColumnPair(@"Noise", @"Noise"),new InputOutputColumnPair(@"Sharpness", @"Sharpness"),new InputOutputColumnPair(@"Blur", @"Blur"),new InputOutputColumnPair(@"Contrast", @"Contrast"),new InputOutputColumnPair(@"EntropyShennon", @"EntropyShennon"),new InputOutputColumnPair(@"EntropyRenyi11", @"EntropyRenyi11"),new InputOutputColumnPair(@"PixelsNum", @"PixelsNum")})      
                                    .Append(mlContext.Transforms.Concatenate(@"Features", new []{@"ChiSqrHorizontalRelativeVolume",@"ChiSqrVerticalRelativeVolume",@"RsRelativeVolume",@"Noise",@"Sharpness",@"Blur",@"Contrast",@"EntropyShennon",@"EntropyRenyi11",@"PixelsNum"}))      
                                    .Append(mlContext.BinaryClassification.Trainers.LightGbm(new LightGbmBinaryTrainer.Options(){NumberOfLeaves=4,NumberOfIterations=3843,MinimumExampleCountPerLeaf=20,LearningRate=0.030308655440333294,LabelColumnName=@"IsDataHided",FeatureColumnName=@"Features",Booster=new GradientBooster.Options(){SubsampleFraction=0.24236556945756227,FeatureFraction=0.8852646195682862,L1Regularization=2E-10,L2Regularization=0.07185978550540768},MaximumBinCountPerFeature=140}));
